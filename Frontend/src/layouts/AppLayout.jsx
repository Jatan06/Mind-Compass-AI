...
export const AppLayout = () => {
  ...
  React.useEffect(() => {
    ...
    if (!authLoading && !isInitialLoading && isAuthenticated && isOnboarded) {
      const hasRedirected = sessionStorage.getItem(
        "has_checked_daily_checkin_redirect",
      );
      if (
        !hasRedirected &&
        userProfile?.notifications?.dailyCheckin !== false
      ) {
        const todayStr = new Date().toISOString().split("T")[0];
        const hasCheckedInToday =
          Array.isArray(checkins) && checkins.some((c) => c.date === todayStr);
  ...
  return (
    <div className="min-h-screen w-full bg-bg-light dark:bg-bg-dark text-text-dark dark:text-text-light flex transition-colors duration-300">
      {/* --- DESKTOP SIDEBAR --- */}
      {!isOnboardingPage && (
        <aside className="hidden md:flex flex-col w-64 bg-bg-light dark:bg-bg-dark border-r border-secondary/15 dark:border-secondary/5 sticky top-0 h-screen overflow-y-auto transition-all select-none shrink-0">
      ...
      <main
        className={`flex-1 min-w-0 bg-bg-light dark:bg-bg-dark ${isOnboardingPage ? "" : "pt-16 md:pt-0"}`}
      >
        <div className="p-6 md:p-10 pb-20 md:pb-10 max-w-7xl w-full mx-auto">
          <Outlet />
        </div>
      </main>
      ...
      <AnimatePresence>{isInitialLoading && <PageLoader />}</AnimatePresence>
    </div>
  );
};
