...
import { FiUser, FiMail, FiEye, FiEyeOff, FiAlertCircle, FiCheckCircle } from 'react-icons/fi';
...
export const RegisterPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (isUserValid && isEmailValid && isPasswordValid && isConfirmValid && isTermsValid) {
            setIsLoading(true);
            try {
                ...
                if (data.success) {
                    ...
                    setSuccessMessage(data.message || 'Registration successful. Verification required.');
                    setRegSuccess(true);
                } else {
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                setResendSuccess('Verification link has been resent. Check your inbox!');
            } else {
            ...
            if (err.response && err.response.data && err.response.data.errors) {
                ...
                else setFormError(err.response.data.message || 'Failed to trigger verification.');
            } else {
    ...
    const handleGoogleSignUp = async () => {
        ...
        try {
            ...
            const response = await authAPI.googleLogin({
                ...
                email: 'google.voyager@example.com',
                name: 'Google Voyager'
            });
            setIsGoogleLoading(false);
            if (response.data.success) {
                navigate('/login', { state: { googleSuccessMessage: 'Google account registered successfully. Please sign in.' } });
            } else {
                setFormError(response.data.message || 'Google registration failed.');
            }
        } catch (err) {
            setIsGoogleLoading(false);
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    ...
                                    {password && (
                                        <div className="space-y-1.5 px-1 pt-1 text-left">
                                            <div className="flex justify-between text-xs">
                                                <span className="text-text-dark/60 dark:text-text-light/65">Password Strength:</span>
                                                <span className="font-semibold text-text-dark dark:text-text-light">{strengthInfo.text}</span>
                                            </div>
                                    ...
                                    <div className="pt-2 text-left">
                                        <label className="flex items-start gap-2.5 text-xs sm:text-sm text-text-dark/70 dark:text-text-light/75 cursor-pointer leading-tight">
                                            ...
                                            <span>
                                                I accept the{' '}
                                                <a href="#" onClick={(e) => { e.preventDefault(); alert('Terms of Service summary...'); }} className="font-semibold text-secondary hover:text-primary dark:hover:text-accent underline">Terms of Service</a>
